...
// ============================================================================
#ifdef _WIN32
char			*vconf = "/Users/shimonaka/Downloads/of_v0.8.0_osx_release/apps/myApps/Shadow_project_single/bin/data/WDM_camera_flipV.xml";
#else
char			*vconf = "";
#endif

char           *cparam_name = "/Users/shimonaka/Downloads/of_v0.8.0_osx_release/apps/myApps/Shadow_project_single/bin/data/camera_para.dat";
ARParam         cparam;

const char *patt_name = "/Users/shimonaka/Downloads/of_v0.8.0_osx_release/apps/myApps/Shadow_project_single/bin/data/patt.hiro";
int             patt_id;
...
void Fpser::Update()
{
	const static double WAIT = 11.1111;
    ...
    if( now_time > wait_time ) time_out = 1;
...
//	cam2.initGrabber(CAM_WIDTH, CAM_HEIGHT);
//	color2.allocate(CAM_WIDTH, CAM_HEIGHT);
//	gray2.allocate(CAM_WIDTH, CAM_HEIGHT);
...
//		color2.setFromPixels(cam2.getPixels(), CAM_WIDTH, CAM_HEIGHT);
		gray2 = color2;
	}
...
//    printf("patt_trans[0][0]=%f\n", patt_trans[0][0]);
//    printf("patt_trans[0][1]=%f\n", patt_trans[0][1]);
//    printf("patt_trans[0][2]=%f\n", patt_trans[0][2]);
    printf("patt_trans[0][3]=%f\n", patt_trans[0][3]);
//    printf("patt_trans[1][0]=%f\n", patt_trans[1][0]);
//    printf("patt_trans[1][1]=%f\n", patt_trans[1][1]);
//    printf("patt_trans[1][2]=%f\n", patt_trans[1][2]);
    printf("patt_trans[1][3]=%f\n", patt_trans[1][3]);
//    printf("patt_trans[2][0]=%f\n", patt_trans[2][0]);
//    printf("patt_trans[2][1]=%f\n", patt_trans[2][1]);
//    printf("patt_trans[2][2]=%f\n", patt_trans[2][2]);
    printf("patt_trans[2][3]=%f\n", patt_trans[2][3]);
    printf("ofGetFrameRate=%f\n", ofGetFrameRate());
    
    if (k != -1) {
        mk_posx = marker_info[k].pos[0] * (glutGet(GLUT_SCREEN_WIDTH) / xsize);
        mk_posy = marker_info[k].pos[1] * 2/* * (glutGet(GLUT_SCREEN_HEIGHT) / (ysize / 1.05))*/;
    }
...
void testApp::draw(){
    ...
    disX = tempX - mk_posx;
    disY = tempY - mk_posy;
    mk_posx = mk_posx + disX;
    mk_posy = mk_posy + disY;
    ...
    if (draw_flag == TRUE) {
        if (ar_flag == 1) {
            ...
            glTranslatef(0, (patt_trans[2][3] / 30) - (1000 - patt_trans[2][3]) / 50, 0);  // tree Y position with ground
            ofTranslate(-mk_posx, -mk_posy);
